...
t('세션 복원 전에는 안 묻는다', /if \(!준비됨 \|\| 지금유저 \|\| 그만묻나\(\)\) \{ resolve\(true\); return; \}/.test(창));
// 등록 시점 값에 갇히면 로그인 뒤에도 계속 뜬다.
t('최신 로그인 상태를 읽는다', /상태\.current = \{ user, isInitialized \};/.test(창));

// ── 한 번만 묻기 ─────────────────────────────────────────────────────────
// 담기 성공 뒤 window.location.reload() 를 하는 화면이 여럿이라,
// 컴포넌트 상태로 기억하면 새로고침마다 지워져 담을 때마다 창이 뜬다.
const 다리 = 읽기('src/utils/guest-prompt.js');
t('그만묻기를 sessionStorage 에 둔다', /sessionStorage\.setItem\(열쇠, '1'\)/.test(다리),
    '컴포넌트 상태로 두면 담기 뒤 새로고침에 지워져 매번 다시 뜬다');
t('저장 실패해도 죽지 않는다', /catch \(e\) \{ return false; \}/.test(다리));
