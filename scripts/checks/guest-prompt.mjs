...
// 주문서도 길목이다 — '장바구니 → 주문하기' 로 오는 손님은 여기서만 만난다.
t('주문서에서도 유도한다', /askGuestSignup\(\);/.test(읽기('src/views/shop/order/OrderSheet.js')),
    "'주문하기' 버튼은 카트 화면 10개에 흩어져 있다 — 도착지인 주문서에 한 번 건다");
