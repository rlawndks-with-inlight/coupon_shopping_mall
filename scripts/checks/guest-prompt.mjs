...
t('배경을 눌러 닫아도 응답한다', /onClose=\{바깥닫기\}/.test(창),
    '응답을 빼면 담기가 영영 안 끝나 버튼이 먹통이 된다');
// 배경·ESC 로 닫는 것은 「비회원으로 계속」이 아니다.
// 제보(2026-08-31, 모바일): 창 밖을 눌렀더니 창만 닫히는 게 아니라 주문서로 넘어갔다.
// 예전엔 onClose 가 비회원으로() 였다 — 스친 손가락 한 번이 곧 결제 진행이었다.
t('바깥을 눌러 닫으면 진행하지 않는다', /const 바깥닫기 = \(\) => \{ 닫기\(false\); \};/.test(창),
    'onClose 를 비회원으로() 에 다시 묶으면 창만 닫으려던 손님이 주문서로 넘어간다');
t('「비회원으로 계속」 버튼만 진행한다', /const 비회원으로 = \(\) => \{ 닫기\(true\); \};/.test(창)
    && /onClick=\{비회원으로\}[\s\S]{0,120}?translate\('비회원으로 계속'\)/.test(창));
