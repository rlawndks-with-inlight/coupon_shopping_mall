...
// ⚠ 물음표가 빠진 /v=2 는 404 다(경로로 읽힌다). 그런데 _app.js 가 어느 페이지든 같은 og 를
//   내보내서 미리보기는 멀쩡해 보이고, 손님만 오류 페이지로 간다. 실제로 그렇게 보낸 사례가 있다.
//   이 경고를 지우면 같은 일이 또 생긴다.
t('물음표를 빠뜨리면 안 된다고 경고한다', /물음표\(\?\)를 꼭 넣으세요/.test(설정));
t('빠뜨렸을 때 무슨 일이 나는지 적는다', /페이지 없음/.test(설정),
    '경고만 하고 결과를 안 적으면 사람은 안 지킨다');
