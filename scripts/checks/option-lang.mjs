...
// ── 백필이 '반쯤 번역된 행' 을 다시 집는가 (2026-08-27) ──────────────────
//
// 번역은 언어별로 실패한다. HTML 본문이 일본어만 되고 영어·중국어·스페인어는 빠지는 일이
// 실제로 있었다(mbc01·테스트01 팝업). 그런데 백필의 '번역 안 된 행' 판정은
// **컬럼 열쇠가 lang_obj 안에 있는지** 만 봤다 — 열쇠는 있으니 '번역됨' 으로 걸러졌고,
// 번역기를 고쳐 놓고 백필을 돌려도 0건이 잡혔다.
// 어느 언어가 찼는지는 SQL 의 LIKE 로 볼 수 없어 판정을 JS 로 옮겼다.
t('백필이 언어 단위로 판정한다', /const 번역이빠졌나 = /.test(backfill),
    'SQL LIKE 로는 어느 언어가 찼는지 볼 수 없다');
t('한 언어라도 비면 다시 담는다', /목표언어\.some\(/.test(backfill));
t('원문이 빈 칸은 건너뛴다', /if \(!String\(item\?\.\[col\] \?\? ''\)\.trim\(\)\) continue;/.test(backfill),
    '번역할 것이 없는 칸까지 담으면 대기열만 늘어난다');
t('lang_obj 를 함께 읽는다', /\$\{table\}\.lang_obj/.test(backfill));
t('대기열에는 원문만 싣는다', /const 원문 = \{ id: item\.id \};/.test(backfill),
    'lang_obj 까지 넣으면 큐 행이 몇 배로 커진다');
