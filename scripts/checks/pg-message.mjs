...
t('결제창을 열기 전 실패는 전부 그 문구를 쓴다',
    실패응답.filter((c) => /결제시작실패문구/.test(c)).length === 7,
    '포스페이 2 · 페이레터 2 · 설정누락 3(페이레터모듈·포스페이모듈·BaseURL) = 7');
...
// 설정 누락도 같은 자리다 — 예전엔 손님 화면에 'MID=client_id, 결제키=API키 입력' ·
// 'FORSPAY_API_BASE 설정이 필요합니다' 가 그대로 떴다. 손님은 알 필요도, 할 수 있는 것도 없다.
t('설정 누락 사유를 손님에게 보내지 않는다',
    !/결제실패응답\([^;]*결제모듈 설정이 필요합니다/s.test(코드)
    && !/결제실패응답\([^;]*FORSPAY_API_BASE/s.test(코드),
    '고쳐야 할 사람은 가맹점·본사다 — 손님이 아니다');
t('설정 누락을 로그에는 남긴다',
    /결제모듈 설정 누락/.test(코드) && /Base URL 미설정/.test(코드),
    '가리기만 하고 안 남기면 왜 결제가 안 되는지 아무도 모른다');
t('로그에 어느 가맹점인지 적는다',
    /설정 누락 — brand_id=/.test(코드),
    '가맹점이 100곳이면 brand_id 없이는 찾을 수 없다');
