...
if (백엔드있음) {
    ...
    t('음수 수량을 저장하지 않고 되돌려 보낸다', /const 수량검사 = /.test(상품2),
        '0 으로 접으면 품절이 된다 — 조용히 고치지 말고 알려야 한다');
    t('create·update 두 자리 모두에서 막는다',
        (상품2.match(/수량검사\(stock_qty, purchase_limit\)/g) ?? []).length === 2);
    t('막는 자리가 저장보다 앞이다',
        상품2.indexOf('수량검사(stock_qty') < 상품2.indexOf('await insertQuery('),
        '저장한 뒤에 막으면 이미 들어간 뒤다 (import 줄이 아니라 실제 호출과 비교한다)');
    const 문구 = '재고와 1인 구매 수량은 0 이상으로 입력해 주세요.';
    t('문구를 조립하지 않는다', 상품2.includes(`'${문구}'`),
        '서버 메시지는 사전에서 글자 그대로 찾는다 — 템플릿으로 만들면 번역이 안 된다');
    for (const lang of ['ko', 'en', 'ja', 'cn', 'es']) {
        t(`${lang} 사전에 수량 문구가 있다`,
            readFileSync(`src/locales/langs/${lang}.js`, 'utf8').includes(`"${문구}"`));
    }
