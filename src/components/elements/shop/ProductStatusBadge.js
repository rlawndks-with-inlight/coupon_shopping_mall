    ...
    <>
      {blocked && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', borderRadius: 'inherit' }}>
          <span style={{ fontWeight: 700, letterSpacing: '0.05em', color: '#555' }}>{s === 2 ? 'SOLD OUT' : '중단됨'}</span>
        </div>
      )}
      <Chip size="small" label={info.text} color={blocked ? 'default' : info.color} variant={blocked ? 'filled' : 'soft'}
