...
export const navConfig = () => {
  ...
  return [
    ...
    //
    // 여기 하위는 전부 조건부다(데모 번호·레벨50·설정값). 팝업관리를 게시판관리로 옮기기
    // 전에는 팝업이 무조건 하나 있어서 빌 일이 없었다. 옮기면서 빌 수 있게 됐다.
    //
    // 어디가 해당되는지는 확인했다(2026-08-24, 하위 브랜드 119개 전수).
    //   · SHOPGO 하위 가맹점: **해당 없음.** 판매 중인 프레임(shop 1·2, blog 1·2·4·9)은
    //     전부 메인페이지관리 아니면 대표 상품·홈 문구가 붙는다. 구조적으로 빌 수 없다.
    //   · 별도 브랜드 7곳(parent_id=null, shop_demo_num 7·8): **여기가 빈다.**
    //     asapmall.kr(8) · the-plusmall.co.kr2 · tjmall.net2 · deoni.co.kr ·
    //     ourshop118855.com · the-plusmall.com · dooncar.com (모두 7)
    //     shop 7·8 은 섹션빌더 목록([1,2,3,4,5,6,9])에 없어 메인페이지관리가 안 뜬다.
    //
    // 빈 채로 두면 눌렀을 때 /manager/designs 로 갔다가 메인페이지관리로 튕긴다 —
    // 그 몰에는 메인페이지관리가 없는데도. 그래서 비면 아예 안 그린다.
    // (팝업관리 자체는 게시판관리에서 그대로 쓴다. 메뉴가 사라지는 것이지 기능이 아니다.)
    ...((() => {
      const 디자인_하위 = [
