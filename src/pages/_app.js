...
App.getInitialProps = async context => {
  ...
  try {
    ...
    if (host) {
      ...
      // ⚠ '대답을 못 받은 것' 과 '없는 몰' 은 다르다 (2026-08-31).
      //
      // API 가 429(요청 과다)나 5xx 를 주면 본문은 `{ data: false }` 다. 그런데 아래 검사는
      // data 가 비었다는 것만 보고 **'등록되지 않은 주소' 로 단정해 404 를 내보냈다.**
      // 실측: 동시 20명이 몰리면 API 속도제한에 걸려 **멀쩡한 몰이 통째로 404** 가 됐다.
      // (속도제한 쪽도 함께 고쳤지만, 백엔드가 잠깐 흔들려도 같은 일이 나므로 여기서도 막는다)
      //
      // 이 조회는 <head> 의 제목·og 태그를 채우는 용도일 뿐이다. 화면에 필요한 브랜드 정보는
      // SettingsContext 가 클라이언트에서 다시 불러온다 — 그래서 여기서 비우고 넘어가도 몰은 뜬다.
      if (!res.ok) {
        return { head_data: {} };
      }
