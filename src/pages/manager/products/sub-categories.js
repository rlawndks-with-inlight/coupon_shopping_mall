...
import { addCategoryByManager, deleteCategoryByManager, getCategoriesByManager, getSubCategoriesByManager, sortCategoryByManager, updateCategoryByManager } from "src/utils/api-manager";
...
const CustomContent = forwardRef(function CustomContent(props, ref) {
  ...
  return (
    ...
    >
      ...
      <Tooltip title={`해당 ${themeDnsData?.setting_obj?.product_sub_category_name}를 수정하시려면 클릭해주세요.`}>
        <IconButton onClick={() => onClickCategoryLabel(category, depth)}>
          <Icon icon='tabler:edit' fontSize={16} />
        </IconButton>
      </Tooltip>
      <Tooltip title={`해당 ${themeDnsData?.setting_obj?.product_sub_category_name}를 한칸 올리시려면 클릭해 주세요.`}>
      ...
      <Tooltip title={`해당 ${themeDnsData?.setting_obj?.product_sub_category_name}를 한칸 내리시려면 클릭해 주세요.`}>
      ...
      <Tooltip title={`해당 ${themeDnsData?.setting_obj?.product_sub_category_name}를 삭제하시려면 클릭해 주세요.`}>
        <IconButton onClick={() => {
          setModal({
...
const CategoryList = () => {
  ...
  const getCategories = async () => {
    let sub_category_list = await getSubCategoriesByManager({
      page: 1,
      page_size: 100000,
    })
                ...
                <Card sx={{ p: 2, height: '100%' }}>
                  {categories.length > 0 ?
                    <>
                      ...
                      <Tooltip title={`새로운 ${themeDnsData?.setting_obj?.product_sub_category_name}를 추가하시려면 클릭해주세요.`} sx={{ margin: 'auto' }} >
                        <Button variant="outlined" sx={{ width: '316px', marginTop: '0.5rem' }} onClick={() => {
                          setIsAction(true);
                          setCategory(defaultSetting)
                          setCurCategories([]);
                        }}>
                          {themeDnsData?.setting_obj?.product_sub_category_name} 추가
                        </Button>
                      </Tooltip>
                    </>
                    :
                        ...
                        {(category?.id || category?.parent || isAction) &&
                          <>
                            <Row>
                              ...
                              {category?.id &&
                                <>
                                  {themeDnsData?.setting_obj?.product_sub_category_name} 수정
                                </>}
                              {category?.parent &&
                                <>
                                  의 하위 카테고리 추가
                              ...
                              {!category?.id && !category?.parent &&
                                <>
                                  새로운 {themeDnsData?.setting_obj?.product_sub_category_name} 추가
                                </>}
                            </Row>
                          </>}
                        <TextField label={`${themeDnsData?.setting_obj?.product_sub_category_name}명`} value={category.category_name} onChange={(e) => {
                          setCategory({
                            ...category,
