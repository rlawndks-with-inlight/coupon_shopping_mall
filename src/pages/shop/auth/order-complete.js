  ...
  return (
    <Wrappers>
      <Stack alignItems="center" spacing={0.5} sx={{ py: 4 }}>
        ...
        <Typography variant="h5" sx={{ fontWeight: 800, mt: 1 }}>{translate('주문이 완료되었습니다')}</Typography>
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>{translate('주문해 주셔서 감사합니다. 아래 주문번호로 조회하실 수 있습니다.')}</Typography>
      </Stack>
      ...
      {order ? (
        ...
        <Card sx={{ mb: 2 }}>
          <CardContent>
            <Typography align="center" sx={{ color: 'text.secondary' }}>{translate('주문이 접수되었습니다. 주문내역에서 상세를 확인해 주세요.')}</Typography>
          </CardContent>
        </Card>
      )}
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
        <Button fullWidth variant="outlined" color="inherit" onClick={() => router.push(isBlogOnly ? '/shop' : '/shop')}>{translate('쇼핑 계속하기')}</Button>
