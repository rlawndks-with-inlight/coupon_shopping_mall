...
const Demo2 = (props) => {
    ...
    const settingPage = () => {
        // TODO: 1:1 문의 백엔드 API가 아직 없습니다.
        // API가 준비되면 여기서 apiManager로 실제 문의 내역을 조회해 setInquiryList로 연결하세요.
        // 예) const res = await apiManager('inquiry/list', { method: 'GET' });
        ...
        <>
            <Wrappers>
                ...
                {inquiryList.filter((item) => item.inquiry_type == inquiryType).length === 0 ? (
                    ...
                    inquiryList.filter((item) => item.inquiry_type == inquiryType).map((item, idx) => (
                        <Accordion
                            key={idx}
