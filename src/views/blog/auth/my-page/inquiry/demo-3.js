...
const Demo3 = (props) => {
    ...
    const settingPage = () => {
        // TODO: 1:1 문의 백엔드 API가 준비되면 여기서 apiManager로 실제 문의 내역을 조회하여
        //       setInquiryList(...)로 채운다. 현재는 관련 API가 존재하지 않아 빈 목록으로 둔다.
        setInquiryList([]);
    }
    ...
    return (
        <>
            <Wrappers>
                ...
                {(() => {
                    const currentList = inquiryList.filter((item) => item.inquiry_type == inquiryType);
