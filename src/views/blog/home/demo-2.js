...
import { Item2, Seller2 } from 'src/components/elements/blog/demo-2';
...
const returnHomeContent = (column, data, func) => {
    ...
    // (blog demo 1·2 본문 폭에 맞춰 demoType=1 — 최대폭 1140px).
            ...
            <ItemContainer>
                {data?.list && data?.list.slice(0, 8).map((item, idx) => (
                    <>
                        {data?.type == 'seller' &&
                            <>
                                <Seller2 item={item} router={router} type={data?.type} />
                            </>}
                        {data?.type == 'items' &&
                            <>
                                <Item2 item={item} router={router} type={data?.type} />
                            </>}
                    </>
                ))}
            </ItemContainer>
        ...
        <>
            <Wrappers>
                ...
                <ItemWrapper>
                    ...
                    {
                        /*
                                            <Row style={{ width: '100%', height: '56px', position: 'fixed', zIndex: '10', background: 'white', maxWidth: '720px' }}>
                                            <div style={{ width: '33.3%', borderBottom: '2px solid #222222', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} onClick={() => { setTab(0) }}>
                                                홈
                                            </div>
                                            <div style={{ width: '33.3%', borderBottom: '2px solid #222222', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} onClick={() => { setTab(1) }}>
                                                스타일
                                            </div>
                                            <div style={{ width: '33.3%', borderBottom: '2px solid #222222', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} onClick={() => { setTab(2) }}>
                                                브랜드관
                                            </div>
                                        </Row>
                        */
