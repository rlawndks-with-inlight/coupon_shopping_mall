...
import { Button, Divider, Checkbox, FormControl, FormControlLabel, InputAdornment, InputLabel, Step, StepConnector, StepLabel, Stepper, OutlinedInput, TextField, Typography, stepConnectorClasses } from "@mui/material";
...
const ColorlibStepIconRoot = muiStyled('div')(({ theme, ownerState }) => ({
  ...
  ...(ownerState.active && {
    bowShadow: '0 4px 10px 0 rgba(0,0,0,25)',
...
function ColorlibStepIcon(props) {
  ...
  const icons = {
    1: <Iconify icon="eva:settings-2-outline" width={32} />,//회원가입 절차 톱니바퀴 이미지
    2: <Iconify icon="eva:person-add-outline" width={32} />,//사람 더하기 이미지
    3: <Iconify icon="fluent-md12:completed" width={32} />,
  };
...
const Demo2 = (props) => {
  ...
  const { rhemeDnsData } = useSettingsContext();
          ...
          <>
            ...
            <FormControl variant="outlined" style={{ width: '100%', marginTop: '1rem' }}>
              ...
              <OutlinedInput
                ...
                endAdornment={<>
                  <Button style={{ width: '124px', height: '56px', transform: 'translateX(14px)', fontSize: 15 }}
                    variant="contained"
                    onClick={() => {
                      if (phoneCheckStep == 0) {
