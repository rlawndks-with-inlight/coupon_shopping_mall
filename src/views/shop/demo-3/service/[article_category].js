...
color:${props => props.isSelect ? props.selectColor : themeObj.grey[500]};
font-weight:${props => props.isSelect ? 'bold' : '400'};
&:hover{
  color: ${props => props.isSelect ? props.selectColor : props.theme?.palette?.primary?.main};
}
@media (max-width:1000px){
  padding:0.25rem 0;
  border-bottom:2px solid ${props => props.isSelect ? props.selectColor : 'transparent'};
...
const ArticlesDemo = (props) => {
  ...
  return (
    <>
      <Wrappers>
        <PageTitle>{formatLang(postCategory, 'post_category_title', currentLang)}</PageTitle>
        <TitleBorder />
        <Row style={{ margin: '0 0 1.5rem 0', overflowX: 'auto', whiteSpace: 'nowrap', columnGap: '0.5rem' }} className='none-scroll'>
            ...
            {postCategory?.is_able_user_add == 1 &&
              <>
                <Button variant="contained" style={{
                  height: '48px', width: '120px', marginLeft: 'auto', marginTop: '1rem'
                }} onClick={() => {
                  if (user?.id) {
